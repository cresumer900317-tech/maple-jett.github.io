document.addEventListener("DOMContentLoaded", async () => {
  ...
  renderWeeklySpotlight(data.weeklyTop?.power || []);
  renderCompactPosts("homeNoticeCompact", notices.slice(0, 4));
  renderCompactPosts("homeTipsCompact", tips.slice(0, 4));
      ...
      <td>
        <div class="character-cell">
          ...
          <div class="character-meta">
            <div class="character-name-row"><span class="name-main">${escapeHtml(row.name || "-")}</span></div>
            <div class="name-sub-row"><span class="name-sub">서버 ${formatNullableRank(row.serverRank)}</span></div>
          </div>
        </div>
      </td>
...
function renderWeeklySpotlight(rows) {
  const root = document.getElementById("homeWeeklyPowerList");
  if (!root) return;
  const preview = rows.slice(0, 10);
  if (!preview.length) {
    root.innerHTML = `<div class="empty-state">주간 성장 데이터가 없습니다.</div>`;
    return;
  }
  root.innerHTML = preview.map((row, index) => {
    const medalClass = getRankBadgeClass(index + 1);
    return `
      <article class="spotlight-card">
        <div class="spotlight-rank ${medalClass}">TOP ${index + 1}</div>
        <div class="spotlight-main">
          <strong>${escapeHtml(row.name || "-")}</strong>
          <span>${escapeHtml(row.guild || "길드 없음")} · 서버 ${formatNullableRank(row.serverRank)}</span>
        </div>
        <div class="spotlight-side">
          <strong class="${getDiffClass(row.diffValue)}">${escapeHtml(row.diffText || "0")}</strong>
          <span>${escapeHtml(row.growthRateText || "0%")}</span>
        </div>
      </article>
    `;
  }).join("");
...
function renderCompactPosts(rootId, posts) {
  ...
  root.innerHTML = posts.map((post) => `
    <article class="board-compact-item">
      <div class="board-compact-top">
        <span class="notice-compact-badge">${escapeHtml(post.category || "게시글")}</span>
        <span class="notice-compact-date">${escapeHtml(formatDateOnly(post.createdAt))}</span>
      </div>
      <div class="notice-compact-title">${post.isPinned ? "📌 " : ""}${escapeHtml(post.title || "-")}</div>
    </article>
  `).join("");
}
  ...
  root.innerHTML = visibleGuilds.map((guild) => `
    <article class="guild-summary-card ${getGuildClass(guild.guild)}" data-guild-name="${escapeHtml(guild.guild || "")}">
      ...
      <div class="guild-summary-stats">
        <div class="stat-tile"><span>인원수</span><strong>${formatNumber(guild.memberCount)}</strong></div>
        <div class="stat-tile"><span>평균 레벨</span><strong>${formatDecimal(guild.avgLevel)}</strong></div>
        <div class="stat-tile"><span>평균 전투력</span><strong>${escapeHtml(guild.avgPowerText || "0")}</strong></div>
        <div class="stat-tile"><span>평균 인기도</span><strong>${formatDecimal(guild.avgPopularity)}</strong></div>
      </div>
    </article>
  `).join("");
...
function openGuildModal(guildName, guilds, members) {
  ...
  meta.innerHTML = `
    <div class="modal-meta-card"><span>인원수</span><strong>${formatNumber(guildInfo?.memberCount || 0)}</strong></div>
    <div class="modal-meta-card"><span>평균 레벨</span><strong>${formatDecimal(guildInfo?.avgLevel || 0)}</strong></div>
    <div class="modal-meta-card"><span>평균 전투력</span><strong>${escapeHtml(guildInfo?.avgPowerText || "0")}</strong></div>
    ...
    list.innerHTML = guildMembers.map((member, index) => `
      <article class="modal-member-card ${getGuildClass(guildName)}">
        <div class="modal-member-top">
          ...
          <span class="modal-member-rank ${getRankBadgeClass(index + 1)}">TOP ${index + 1}</span>
        </div>
        <div class="modal-member-stats">
          <div class="modal-member-stat"><span>레벨</span><strong>${formatNumber(member.level)}</strong></div>
          <div class="modal-member-stat"><span>전투력</span><strong>${escapeHtml(member.powerText || "0")}</strong></div>
